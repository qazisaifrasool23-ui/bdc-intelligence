...
from common import (
    ...
    MATERIAL_8K_ITEMS, FORM_LABELS, COVERAGE, CIK_OVERRIDES,
)
...
def run(mode):
    ensure_dirs()
    funds = load_funds()
    # Restrict the news feed to the 20 market-movers; apply CIK overrides.
    cov = set(COVERAGE)
    funds = [f for f in funds if f["ticker"] in cov]
    for f in funds:
        if not f.get("cik") and f["ticker"] in CIK_OVERRIDES:
            f["cik"] = CIK_OVERRIDES[f["ticker"]]
    have = {f["ticker"] for f in funds}
    missing = [t for t in COVERAGE if t not in have]
    if missing:
        log.warning("no CIK for coverage funds (add to CIK_OVERRIDES): %s", ", ".join(missing))
    log.info("news coverage: %d of 20 funds have a CIK", len(funds))
    llm = LLM()
